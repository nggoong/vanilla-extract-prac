...
### 코드 스플리팅에 대한 단점을 극복하자.

> createThemeContract()를 통해 정적 타이핑을 위한 theme contract를 만들 수 있다.

createTheme을 통해 선언된 코드로 인해 css가 생성됨과는 다르게 createThemeContract는 css를 생성하지 않는다. 따라서 단순히 정적 타이핑을 위한 contract를 만든다고 생각하면 된다.
